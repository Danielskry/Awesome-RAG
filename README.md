...
- [LocalMode](https://github.com/LocalMode-AI/LocalMode): Local-first, privacy-first RAG framework that runs entirely in the browser (embeddings, vector DB with HNSW index, semantic cache, and agents), with no servers or API keys and data that never leaves the device.
