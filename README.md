...
- **[ragbench](https://github.com/catidegla/ragbench)**: CI gate that scores a run against a golden dataset and fails the build on a regression against a stored baseline. Deterministic metrics only, so no LLM judge and no API key.
