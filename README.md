...
- [Scrapiq](https://github.com/NG-PR0JECT/scrapiq): Open-source API that turns any URL into clean, structured JSON (title, content, links, metadata) for RAG ingestion pipelines — MIT-licensed, self-hostable with one Docker command, no LLM in the extraction path.
