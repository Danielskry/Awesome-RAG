...
- [repo2graph](https://github.com/Srinivasan-78/repo2graph): Zero-dependency GraphRAG for code — parses a repo with tree-sitter into a typed symbol graph (DEFINES/IMPORTS/CALLS/INHERITS), then answers a query with hybrid BM25+vector retrieval expanded across that graph, packed into cited, token-budgeted context. CLI and MCP server, no required embedding API.
